    ...
    try:    
        ...
        if blank == 1:
            while True:
                ...
                ranNum = random.sample(range(len(nouns)),1) # 0~명사 길이 범위 에서 랜덤 숫자 1개 추출
                line = line.replace(nouns[ranNum[0]], '( )') # word와 일치 하는 단어를 ()로 교체 해서 다시 저장           
                f2.write(line)        
...
makeTwoBlanks('indext2.txt','new.txt',2) #빈칸 1개 생성할 경우
